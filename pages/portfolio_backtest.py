...
def search_symbols_portfolio(search_term: str) -> list[str]:
    """
    Calls the search_symbols method of the currently selected data source for the portfolio page.
...
# --- UI Elements and Logic for the Portfolio Backtest Page ---
# This code will be executed directly by Streamlit when the page is selected.

st.title("Portfolio-Level Backtest")
...
st.header("Portfolio Backtest Parameters")

# Use a container for Data Source, Date Range, Initial Capital, and Symbol Selection
with st.container(border=True):
    ...
    with col_date_start:
        ...
        start_date = st.date_input("Start Date", today - datetime.timedelta(days=365*2), key='portfolio_start_date')
    with col_date_end:
        end_date = st.date_input("End Date", today, key='portfolio_end_date')

    # Initial Capital - Moved here
    initial_capital = st.number_input("Initial Capital", min_value=1000.0, value=100000.0, step=1000.0, key='portfolio_initial_capital')


    st.subheader("Symbol Selection")
    st.write("Select symbols for portfolio backtesting:")
    num_symbols = st.number_input("Number of Symbols", min_value=1, value=2, step=1, key='portfolio_num_symbols') # Default to 2 for portfolio
    selected_symbols = []
    for i in range(num_symbols):
        symbol = st_searchbox(
            search_symbols_portfolio, # Use the dynamic search function
            label=f"Search and Select Symbol {i+1}",
            key=f"portfolio_symbol_searchbox_{i}"
        )
        if symbol:
...
# Removed: Initial Portfolio Allocation section


# Use a container for Strategy and Parameters
with st.container(border=True):
    st.subheader("Strategy Configuration")
    selected_strategy_name = st.selectbox("Select Strategy", list(AVAILABLE_STRATEGIES.keys()), key='portfolio_strategy')
                    ...
                    if default_params:
                        ...
                        for param_name, default_value in default_params.items():
                            ...
                            with current_col:
                                ...
                                current_value = st.session_state.get(f'portfolio_param_{selected_strategy_name}_{param_name}', default_value)

                                if isinstance(default_value, int):
                                    strategy_params[param_name] = st.number_input(param_name, min_value=1, value=int(current_value), step=1, key=f'portfolio_param_{selected_strategy_name}_{param_name}_int')
                                elif isinstance(default_value, float):
                                     strategy_params[param_name] = st.number_input(param_name, value=float(current_value), step=0.1, format="%.2f", key=f'portfolio_param_{selected_strategy_name}_{param_name}_float')
                                elif isinstance(default_value, bool):
                                     strategy_params[param_name] = st.checkbox(param_name, value=bool(current_value), key=f'portfolio_param_{selected_strategy_name}_{param_name}_bool')
                                else:
                                     strategy_params[param_name] = st.text_input(param_name, str(current_value), key=f'portfolio_param_{selected_strategy_name}_{param_name}_text')
                                # Store parameter value in session state for persistence
                                st.session_state[f'portfolio_param_{selected_strategy_name}_{param_name}'] = strategy_params[param_name]
...
with st.container(border=True):
    ...
    with col_sl:
        stop_loss_pct = st.number_input("Global Stop Loss (%)", min_value=0.0, max_value=100.0, value=0.0, step=0.1, format="%.2f", key='portfolio_stop_loss_pct') / 100.0
    with col_tp:
        take_profit_pct = st.number_input("Global Take Profit (%)", min_value=0.0, value=0.0, step=0.1, format="%.2f", key='portfolio_take_profit_pct') / 100.0

    st.subheader("Position Sizing")
    position_sizing_method = st.selectbox(
        "Method",
        ['Full Equity', 'Fixed Amount', 'Percentage of Equity'],
        key='portfolio_position_sizing_method'
    )
    position_sizing_value = 0.0
    if position_sizing_method == 'Fixed Amount':
        position_sizing_value = st.number_input("Amount ($)", min_value=1.0, value=1000.0, step=100.0, key='portfolio_position_sizing_amount')
    elif position_sizing_method == 'Percentage of Equity':
        position_sizing_value = st.number_input("Percentage (%)", min_value=0.1, max_value=100.0, value=10.0, step=0.1, format="%.2f", key='portfolio_position_sizing_percentage') / 100.0
...
is_disabled = not selected_symbols or strategy_instance is None or (position_sizing_method != 'Full Equity' and position_sizing_value <= 0)
...
# Check if allocation is valid before enabling the button
# The check for selected_symbols is now part of the main is_disabled check
# if selected_symbols and abs(total_allocated_pct - 100.0) > 1e-6:
#      # If symbols are selected but allocation is not 100%, allow running but warn.
#      # The backtester will treat the difference as cash.
#      pass # Button remains enabled if other conditions are met
# elif not selected_symbols:
#      is_disabled = True # Disable if no symbols selected


if st.button("Run Portfolio Backtest", key='run_portfolio_backtest_main', disabled=is_disabled):
    if start_date >= end_date:
        ...
        try:
            ...
            if "error" in results:
                ...
                if 'last_portfolio_backtest_results' in st.session_state:
                    del st.session_state['last_portfolio_backtest_results']
                if 'last_portfolio_backtest_config' in st.session_state: # Also clear config on backtest error
                     del st.session_state['last_portfolio_backtest_config']
            else:
                st.success("Backtest completed.")
                st.toast("Backtest completed successfully!", icon="✅")

                # Store results in session state for saving
                st.session_state['last_portfolio_backtest_results'] = results
                # Store current configuration used for this backtest run for saving purposes
                st.session_state['last_portfolio_backtest_config'] = {
                ...
                if 'portfolio_history' in results and results['portfolio_history'] is not None and 'total' in results['portfolio_history']:
                    ...
                    if isinstance(portfolio_total, pd.Series) and not portfolio_total.empty:
                         ...
                         if portfolio_total is not None:
                              ...
                              equity_curves_for_plotting = {f"{selected_strategy_name} (Portfolio)": portfolio_total}
                              try:
                ...
                # Removed: Display Detailed Portfolio History section
                ...
                if 'trades' in results and results['trades']:
                    with st.expander("View Trade Log (All Assets)"):
                        ...
                        if isinstance(results['trades'], list) and all(isinstance(trade, dict) for trade in results['trades']):
                            ...
                            st.dataframe(trades_df, key='portfolio_trade_log')
                        else:
            ...
            if 'last_portfolio_backtest_results' in st.session_state:
                del st.session_state['last_portfolio_backtest_results']
            if 'last_portfolio_backtest_config' in st.session_state:
                 del st.session_state['last_portfolio_backtest_config']
...
if 'last_portfolio_backtest_results' in st.session_state and st.session_state.get('last_portfolio_backtest_results') is not None \
   and 'last_portfolio_backtest_config' in st.session_state and st.session_state.get('last_portfolio_backtest_config') is not None:

    results_to_save = st.session_state['last_portfolio_backtest_results']
    config_used = st.session_state['last_portfolio_backtest_config']
    ...
    # Removed: Detailed Portfolio History DataFrame preparation for download
    ...
    with save_button_placeholder.container():
        ...
        col_metrics, col_equity, col_trades = st.columns(3) # Removed column for detailed history
        ...
        with col_metrics:
            st.download_button(
                ...
                key='download_portfolio_metrics_json',
        ...
        with col_trades:
            ...
            if not trades_df.empty and "Error" not in trades_df.columns:
                st.download_button(
                    ...
                    mime="text/csv",
                    key='download_portfolio_trades_csv',
